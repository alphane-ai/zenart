      ...
      <section className="panel">
        ...
        <DataTable<StagingObjectStorageRetentionCleanupEvidence>
          ...
          columns={[
            { key: "id", header: "Evidence", render: (row) => <span className="mono">{row.id}</span> },
            { key: "status", header: "Status", render: (row) => <StatusBadge value={row.status} label={row.status} /> },
            { key: "kind", header: "Report Kind", render: (row) => <StatusBadge value={row.reportKind === "canonical_pass" ? "approved" : row.reportKind === "missing" ? "warning" : "blocked"} label={row.reportKind} /> },
            { key: "check", header: "Release Gate Check", render: (row) => row.releaseGateCheckId },
            ...
            { key: "canonical", header: "Canonical Pass Report", render: (row) => row.canonicalPassReportPath },
            { key: "blocked-probe", header: "Blocked Probe Report", render: (row) => row.blockedProbeReportPath },
            { key: "observed", header: "Observed Report", render: (row) => row.observedReportPath },
