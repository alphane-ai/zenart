      ...
      <section className="panel">
        ...
        <DataTable<CrawlerGovernanceAdminActionContract>
          ...
          columns={[
            ...
            { key: "session", header: "Admin Session Scope", render: (row) => row.adminSessionScope },
            { key: "attempt", header: "Request Attempt Ref", render: (row) => <span className="mono">{row.requestAttemptRef}</span> },
            { key: "idempotency", header: "Idempotency Key", render: (row) => <span className="mono">{row.idempotencyKey}</span> },
            { key: "digest", header: "Request State Digest", render: (row) => <span className="mono">{row.requestStateDigest}</span> },
            { key: "stale", header: "Stale Replay Outcome", render: (row) => <StatusBadge value="blocked" label={row.staleReplayOutcome} /> },
            { key: "audit-order", header: "Request Audit Order", render: (row) => row.requestAuditOrder },
            { key: "request-evidence", header: "Request Evidence Refs", render: (row) => row.requestEvidenceRefs.join(", ") },
