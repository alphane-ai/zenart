...
var launchCrawlerResearchSecretKeyPattern = regexp.MustCompile(`(?i)(firecrawl[_-]?(?:api[_-]?key|token|secret|webhook[_-]?secret)|tavily[_-]?(?:api[_-]?key|token|secret)|exa[_-]?(?:api[_-]?key|token|secret)|serpapi[_-]?(?:api[_-]?key|token|secret)|serper[_-]?(?:api[_-]?key|token|secret)|apify[_-]?(?:api[_-]?key|token|secret)|bright[_-]?data[_-]?(?:api[_-]?key|token|password|proxy[_-]?password|customer[_-]?id)|zyte[_-]?(?:api[_-]?key|token|password)|scrapingbee[_-]?(?:api[_-]?key|token)|scraperapi[_-]?(?:api[_-]?key|token)|crawlbase[_-]?(?:api[_-]?key|token)|diffbot[_-]?(?:api[_-]?key|token)|jina[_-]?(?:api[_-]?key|token)|searchapi[_-]?(?:api[_-]?key|token|secret)|crawler[_-]?(?:api[_-]?key|token|secret|proxy[_-]?password|proxy[_-]?authorization|webhook[_-]?secret)|web[_-]?research[_-]?(?:api[_-]?key|token|secret))`)
var launchCrawlerResearchSecretAssignmentPattern = regexp.MustCompile(`(?i)\b([A-Za-z0-9_.-]*(?:firecrawl[_-]?(?:api[_-]?key|token|secret|webhook[_-]?secret)|tavily[_-]?(?:api[_-]?key|token|secret)|exa[_-]?(?:api[_-]?key|token|secret)|serpapi[_-]?(?:api[_-]?key|token|secret)|serper[_-]?(?:api[_-]?key|token|secret)|apify[_-]?(?:api[_-]?key|token|secret)|bright[_-]?data[_-]?(?:api[_-]?key|token|password|proxy[_-]?password|customer[_-]?id)|zyte[_-]?(?:api[_-]?key|token|password)|scrapingbee[_-]?(?:api[_-]?key|token)|scraperapi[_-]?(?:api[_-]?key|token)|crawlbase[_-]?(?:api[_-]?key|token)|diffbot[_-]?(?:api[_-]?key|token)|jina[_-]?(?:api[_-]?key|token)|searchapi[_-]?(?:api[_-]?key|token|secret)|crawler[_-]?(?:api[_-]?key|token|secret|proxy[_-]?password|proxy[_-]?authorization|webhook[_-]?secret)|web[_-]?research[_-]?(?:api[_-]?key|token|secret))[A-Za-z0-9_.-]*)\s*([=:])\s*("[^"]*"|'[^']*'|[^\s,;&]+)`)
...
func ClassifyKey(key string) []SecretFinding {
	...
	case launchCrawlerResearchSecretKeyPattern.MatchString(key) || launchCrawlerResearchSecretKeyPattern.MatchString(normalized):
		switch {
		case strings.Contains(lower, "webhook"):
			kind = SecretKindWebhookSecret
		case strings.Contains(lower, "password"):
			kind = SecretKindPassword
		case strings.Contains(lower, "proxy_authorization"):
			kind = SecretKindAuthorization
		case strings.Contains(lower, "api_key"):
			kind = SecretKindAPIKey
		default:
...
func ClassifyString(value string) []SecretFinding {
	...
	for _, match := range launchCrawlerResearchSecretAssignmentPattern.FindAllStringSubmatch(value, -1) {
		for _, keyFinding := range ClassifyKey(match[1]) {
			keyFinding.Signal = "assignment:" + keyFinding.Signal
			findings = append(findings, keyFinding)
		}
