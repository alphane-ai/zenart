...
# target_host=52.237.80.117
# password_persisted=false
# public_key_fingerprint=SHA256:A/ZVZBi33Yf3DdiwqDdhxlQYQsPKXjjLDY3s66VcDR4

USER_NAME='sansha'
PUBKEY='ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJfAq7GQ+IfFEvBPG5SZmh56tuJDzKhUiDcoqUAKvYJn mac@macdeMac-mini.local-machine-login-20260512'
PUBLIC_KEY_FINGERPRINT='SHA256:A/ZVZBi33Yf3DdiwqDdhxlQYQsPKXjjLDY3s66VcDR4'
...
if [ -f /opt/zenari/current/docker-compose.yml ]; then
  ...
  origin_container_env() {
    ...
    $docker_cmd inspect --format '{{range .Config.Env}}{{println .}}{{end}}' "$cid" 2>/dev/null       | awk -F= -v key="$key" '$1 == key {print substr($0, length(key) + 2); exit}'
  }
