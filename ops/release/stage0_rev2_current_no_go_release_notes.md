...
- CI gate: CI gate `no_go` from `fixtures/stage0/rev2/release_gate_evidence.ci.json`; blocked checks: ci_gate_runtime_execution, ci_playwright_smoke, ci_docker_image_build; active do-not-launch conditions: ci_gate_not_executed_on_main, ci_playwright_smoke_missing, ci_docker_image_build_missing; exact closure artifacts: installed PR/main workflow `.github/workflows/stage0-rev2-ci.yml` present; PR/main workflow run evidence `ops/evidence/ci/stage0-rev2-pr-main-run.json` absent; CI Playwright smoke evidence `ops/evidence/ci/stage0-rev2-playwright-smoke.json` absent; CI Docker image build evidence `ops/evidence/ci/stage0-rev2-docker-image-build.json` absent.
...
- Release evidence bundle: `blocked` / `no-go` from `ops/evidence/release/staging/stage0-rev2-current-release-evidence-bundle.json` with 39 blocking reasons; legal/support source `canonical_staging_split_evidence`; object-retention cleanup remains unverified and canonical pass evidence is still required.
...
- CI gate: `no_go` from `fixtures/stage0/rev2/release_gate_evidence.ci.json`; blocked checks: ci_gate_runtime_execution, ci_playwright_smoke, ci_docker_image_build; active do-not-launch conditions: ci_gate_not_executed_on_main, ci_playwright_smoke_missing, ci_docker_image_build_missing; decision evidence: fixtures/stage0/rev2/release_gate_evidence.ci.json gate_decision.status=no_go for checklist item CI installed workflow runtime evidence 通过：PR/main run、Playwright smoke、Docker image build 均有 validator-resolvable evidence。 Installed workflow artifact .github/workflows/stage0-rev2-ci.yml passes file evidence. ci_gate_runtime_execution, ci_playwright_smoke, and ci_docker_image_build are blocked while ci_gate_not_executed_on_main, ci_playwright_smoke_missing, and ci_docker_image_build_missing remain active; exact PR/main run runtime evidence remains absent/missing at ops/evidence/ci/stage0-rev2-pr-main-run.json; exact CI Playwright smoke runtime evidence remains absent/missing at ops/evidence/ci/stage0-rev2-playwright-smoke.json; exact CI Docker image build runtime evidence remains absent/missing at ops/evidence/ci/stage0-rev2-docker-image-build.json
- Private Beta/Staging gate: `no_go` from `fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json`; blocked checks: staging_object_storage_signed_downloads; active do-not-launch conditions: object_storage_signed_retention_runtime_missing; decision evidence: fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json gate_decision.status=no_go for checklist item Private Beta/Staging external-user runtime evidence 通过：auth/RBAC/tenant、storage、quota/rate limit、support/abuse、safety/QA/crawler、observability/backup/load、legal visibility 均有 staging evidence。 while staging_object_storage_signed_downloads is blocked and object_storage_signed_retention_runtime_missing remains active. Required retention cleanup artifact ops/evidence/staging/object-storage-retention-cleanup.json is absent and must cover retention policy, expired export cleanup, orphan cleanup, and audit refs. Legal/support external-user visibility passed in ops/evidence/staging/legal-pages-external-user.json and ops/evidence/staging/support-contact-external-user.json. Staging observability backup load passed in ops/evidence/staging/20260527T013207Z-staging-observability-backup-load-36222.json with logs, metrics, traces, alerts, dashboard, restore, load, and post-deploy smoke evidence from ops/evidence/staging/20260527T1830Z-observability-runtime.json, ops/evidence/staging/20260527T2115Z-backup-restore.json, ops/evidence/staging/20260527T2120Z-load.json, and ops/evidence/staging/20260527T2125Z-post-deploy-smoke.json. Object-storage signed URL evidence passed in ops/evidence/staging/20260527T2130Z-object-storage-signed-url.json and proves tenant-scoped signed download, expiry, direct-object denial, and cross-tenant denial.
- Production Launch gate: `no_go` from `fixtures/stage0/rev2/release_gate_evidence.production_launch.json`; blocked checks: production_paid_billing_lifecycle, production_backup_rollback_incident; active do-not-launch conditions: paid_billing_or_comp_only_mode_missing, backup_restore_rollback_smoke_missing, production_deploy_rollback_smoke_missing, ci_staging_gates_not_passed; decision evidence: fixtures/stage0/rev2/release_gate_evidence.production_launch.json gate_decision.status=no_go for checklist item Production Launch runtime/deployment evidence 通过：provider-or-comp-only、paid lifecycle、skill canary、activation audit、abuse hold、security、backup/rollback/post-deploy smoke、legal/support policy 均有 production evidence。 while production_paid_billing_lifecycle and production_backup_rollback_incident are blocked and paid_billing_or_comp_only_mode_missing, backup_restore_rollback_smoke_missing, production_deploy_rollback_smoke_missing, and ci_staging_gates_not_passed remain active; ops/evidence/production/provider-mode.json plus ops/evidence/production/public-paid-real-generation-claims.json clear provider mode through invite/comp-only mode; ops/evidence/production/billing-lifecycle.json is present deferred Stripe no-go evidence for checkout/subscription/cancellation/past_due proof; ops/evidence/production/billing-refund-credit-webhook.json is present deferred Stripe no-go evidence for refund/quota reset/webhook proof; paid lifecycle remains blocked until real Stripe checkout/subscription/cancellation/past_due/refund/quota reset/webhook proof exists; ops/evidence/production/20260527T1800Z-backup-rollback-incident-smoke.json is admin-visible operations evidence but cannot close the backup check before upstream gates pass; exact production backup/restore evidence ops/evidence/production/backup-restore.json is absent; exact production rollback/incident/post-deploy smoke evidence ops/evidence/production/rollback-incident-post-deploy-smoke.json is absent; present upstream CI fixture fixtures/stage0/rev2/release_gate_evidence.ci.json gate_decision.status=no_go; present upstream Private Beta/Staging fixture fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json gate_decision.status=no_go
- Global Do-Not-Launch Conditions: `open`; non-go gate decisions: fixtures/stage0/rev2/release_gate_evidence.ci.json gate_decision.status=no_go, fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json gate_decision.status=no_go, fixtures/stage0/rev2/release_gate_evidence.production_launch.json gate_decision.status=no_go; active conditions: fixtures/stage0/rev2/release_gate_evidence.ci.json condition_id=ci_gate_not_executed_on_main is_present=true, fixtures/stage0/rev2/release_gate_evidence.ci.json condition_id=ci_playwright_smoke_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.ci.json condition_id=ci_docker_image_build_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json condition_id=object_storage_signed_retention_runtime_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.production_launch.json condition_id=paid_billing_or_comp_only_mode_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.production_launch.json condition_id=backup_restore_rollback_smoke_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.production_launch.json condition_id=production_deploy_rollback_smoke_missing is_present=true, fixtures/stage0/rev2/release_gate_evidence.production_launch.json condition_id=ci_staging_gates_not_passed is_present=true; required closure decisions: fixtures/stage0/rev2/release_gate_evidence.local_alpha.json gate_decision.status=go, fixtures/stage0/rev2/release_gate_evidence.ci.json gate_decision.status=go, fixtures/stage0/rev2/release_gate_evidence.private_beta_staging.json gate_decision.status=go, fixtures/stage0/rev2/release_gate_evidence.production_launch.json gate_decision.status=go; checklist row `Do-Not-Launch Conditions 全部为 false。` remains open until every release gate fixture computes `gate_decision.status=go` and no fixture has `is_present=true` do-not-launch conditions.
...
- Open CI blockers: `fixtures/stage0/rev2/release_gate_evidence.ci.json`: ci_gate_runtime_execution, ci_playwright_smoke, ci_docker_image_build.
- CI do-not-launch conditions present: ci_gate_not_executed_on_main, ci_playwright_smoke_missing, ci_docker_image_build_missing.
...
- Open production blockers: `fixtures/stage0/rev2/release_gate_evidence.production_launch.json`: production_paid_billing_lifecycle, production_backup_rollback_incident.
- Production do-not-launch conditions present: paid_billing_or_comp_only_mode_missing, backup_restore_rollback_smoke_missing, production_deploy_rollback_smoke_missing, ci_staging_gates_not_passed.
...
- CI and staging runtime open target rows: CI 在已安装 PR/main workflow 中运行 Playwright smoke; CI 在已安装 PR/main workflow 中 build Docker images. These are blueprint checklist labels that remain unchecked, not satisfied release evidence.
