...
9. Run `scripts/staging_object_storage_retention_cleanup_smoke.sh` against staging admin object-storage retention and cleanup endpoints. Passing evidence must be written to `ops/evidence/staging/object-storage-retention-cleanup.json` and prove retention policy, expired export cleanup, orphan cleanup, and audit refs before the object-storage gate can close.
