#!/usr/bin/env python3
"""Normalize a passed Docker build smoke report into exact Stage 1 CI evidence."""
...
DEFAULT_OUTPUT_REF = "ops/evidence/ci/stage0-rev2-docker-image-build.json"
...
class DockerEvidenceError(Exception):
    pass


def require(condition: bool, message: str) -> None:
    if not condition:
        raise DockerEvidenceError(message)
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise DockerEvidenceError(f"{display_path(path)} invalid JSON: {exc}") from exc
...
def normalize_images(report: dict[str, Any]) -> dict[str, Any]:
    images = report.get("images")
    require(isinstance(images, dict), "source report images must be an object")
    missing = set(REQUIRED_IMAGES) - set(images)
    require(not missing, f"source report missing image sections: {sorted(missing)}")
    normalized: dict[str, Any] = {}
    for name in REQUIRED_IMAGES:
        image = images.get(name)
        require(isinstance(image, dict), f"images.{name} must be an object")
        status = normalize_status(image.get("status"))
        digest = image.get("digest")
        require(isinstance(digest, str) and digest.startswith("sha256:") and len(digest) >= 71, f"images.{name}.digest must be immutable sha256 digest")
        refs = require_ref_list(image.get("evidence_refs"), f"images.{name}.evidence_refs")
        row: dict[str, Any] = {
            "status": status,
            "digest": digest,
            "evidence_refs": refs,
        }
        for optional in ("tag", "context", "target"):
            if isinstance(image.get(optional), str):
                row[optional] = image[optional]
        normalized[name] = row
    return normalized
...
def build_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    evidence: dict[str, Any] = {
        ...
        "image_prefix": report.get("image_prefix"),
        "image_set": report.get("image_set"),
        "evidence_refs": evidence_refs,
        "images": normalize_images(report),
        "gate_impact": {
            "release_gate_check_id": "ci_docker_image_build",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Write exact Stage 1 CI Docker image build evidence")
    parser.add_argument("--source", type=Path, required=True, help="source JSON from scripts/docker_build_smoke.sh")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="canonical CI Docker evidence path")
...
def main() -> int:
    ...
    try:
        ...
        print(f"write Stage 1 CI Docker evidence failed: {exc}", file=sys.stderr)
        return 1
    print(f"wrote Stage 1 CI Docker evidence to {display_path(args.output)}")
    return 0
