...
required_fragments = [
    ...
    "Production Launch gate: `no_go` from `fixtures/stage0/rev2/release_gate_evidence.production_launch.json`; blocked checks: production_paid_billing_lifecycle, production_backup_rollback_incident; active do-not-launch conditions: paid_billing_or_comp_only_mode_missing, backup_restore_rollback_smoke_missing, production_deploy_rollback_smoke_missing, ci_staging_gates_not_passed;",
...
cat >"$object_retention_canonical_report" <<EOF
{
  ...
  "csrf": {
    "ready": true,
    "origin": "https://admin.staging.example.test",
    "header_name": "X-ZenArt-CSRF"
  },
