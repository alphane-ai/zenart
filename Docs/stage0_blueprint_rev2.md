...
- [x] Stage0 export retrieval fails closed without the backend-mediated download signer and never falls back to provider-native object-store presigned URLs; service tests cover persisted object-key signing through the backend signer and no direct object-store signer invocation.
- [x] Backend-mediated signed upload/download URLs reject duplicate `key`、`expires`、or `sig` query parameters before tenant-scoped storage, audit, or analytics side effects, with server tests covering fail-closed upload and download paths.
...
- [x] Eval result admin projection contract 通过：`fixtures/stage0/rev2/eval/eval_storage_contract.json` declares `EvalResult`/`EvalResultPage` projection of result_id、summary、fixture QA/trace/export gates、runner hash、exact source fixture digests、storage immutability/retention/read_without_eval_rerun fields, and `scripts/validate_eval_storage_contract.py` validates the OpenAPI projection against the stored eval result fixture。
- [x] Eval result artifact retrieval contract 通过：`fixtures/stage0/rev2/eval/eval_result_artifact_contract.json` binds stored eval result JSON artifact digests、source fixture digests、runner manifest、QA/safety/trace gate refs, and `scripts/validate_eval_result_artifact_contract.py` validates admin-only tenant-scoped signed retrieval through OpenAPI without rerunning eval。
- [x] Eval runner manifest contract 通过：`fixtures/stage0/rev2/eval/eval_runner_manifest_contract.json` binds exact runner hash、runner manifest hash、source fixture digests、stale-input rejection cases, and `scripts/validate_eval_runner_manifest_contract.py` validates replay without accepting stale eval inputs。
...
- [x] Safety effective decision precedence contract 通过：`fixtures/stage0/rev2/eval/safety_enforcement_contract.json` declares per-point allow/warn/user-confirm/admin-review/block aggregation cases, and `scripts/run_safety_policy_runtime_contract.py` verifies final export uses the strongest persisted safety decision across brief、provider request、provider response、QA、export。
- [x] Export override replay contract 通过：`fixtures/stage0/rev2/eval/export_override_contract.json` declares approved/denied QA、safety_decision、export_contract override outcomes, and `scripts/run_export_override_contract.py --check` replays eval result、QA source、safety source、trace/export links while denying critical safety、incomplete export artifact、missing audit overrides。
- [x] Eval package readiness contract 通过：`fixtures/stage0/rev2/eval/eval_package_readiness_contract.json` binds exact eval runner replay、source fixture digests、QA/safety/trace links、package/export IDs、blocked-export retention, and `scripts/validate_eval_package_readiness_contract.py` validates package download gating from stored eval results without stale fixture drift。
- [x] Export eligibility decision contract 通过：`fixtures/stage0/rev2/eval/export_eligibility_decision_contract.json` cross-checks eval pass、candidate generation、QA coverage/blockers、safety holds、trace completeness、package readiness、export artifacts、override eligibility、and an explicit positive allow-download path before enabling package download, enforced by `scripts/validate_export_eligibility_decision_contract.py`。
...
- [x] 每条 workflow safety coverage contract 通过：`fixtures/stage0/rev2/eval/workflow_safety_coverage_contract.json` maps required safety domains to safety rules、eval fixtures、all five enforcement points、trace safety decisions、and export gates, enforced by `scripts/validate_workflow_safety_coverage_contract.py`。
...
- [x] Web CSRF/same-site session transition UX evidence 通过：`web/components/workspace-app.tsx` exposes transition digest for authenticated -> expired -> recovered, expired -> sign-in recovery, authenticated -> signed_out, and signed_out -> sign-in; `web/validation/user-routes-smoke.json` and `web/validation/session-security-browser-contract.json` pin expired-only Refresh Session recovery and signed-out guarded-control blocking, validated by Vitest、Playwright session-security smoke、and `npm run smoke:security-contracts`。
...
- [x] 扩展 launch backup/incident secret redaction/classification 覆盖 Restic、Kopia、Borg、pgBackRest、WAL-G、Litestream、age/GPG/PGP backup keys and Rootly、FireHydrant、Statuspage、VictorOps、Splunk On-Call、Squadcast incident tokens，避免 backup/rollback/smoke evidence、audit/support/export/crawler metadata 泄露恢复和事故响应凭据，同时保留 public status-page component metadata.
- [x] 扩展 launch infrastructure secret redaction/classification 覆盖 Redis、SMTP、Elasticsearch/OpenSearch、Elastic Cloud、ClickHouse、Metabase、Grafana service-account/API-key 字符串和结构化 metadata，避免 logs/traces/errors/audit/support/export/analytics 泄露 runtime infrastructure credentials.
- [x] 扩展 signed delivery secret redaction/classification 覆盖 CloudFront signed cookies、Google Cloud CDN signed URL/cookie fields、generic CDN token fields、Cloudflare Access JWT assertion 的字符串 assignment 和结构化 metadata，同时保留 public response override metadata.
- [x] 扩展 launch data/warehouse secret redaction/classification 覆盖 Snowflake、BigQuery/GCP service-account、Databricks、dbt Cloud、Airbyte、Fivetran、MotherDuck/DuckDB、Neon、PlanetScale、Turso/libSQL、Aiven、CockroachDB、MongoDB、Postgres、MySQL 的字符串 assignment 和结构化 metadata，避免 logs/traces/errors/audit/support/export/crawler/analytics 泄露数据平面凭据.
- [x] 扩展 launch package registry/SCM secret redaction/classification 覆盖 npm/Yarn/pnpm、PyPI/Twine、RubyGems、Cargo/crates.io、Composer/Packagist、NuGet、Maven/Gradle、JFrog/Artifactory、Cloudsmith、GitLab deploy token、Gitea deploy key、Bitbucket app password、Sourcegraph、Gerrit HTTP password 的字符串 assignment 和结构化 metadata，避免 logs/traces/errors/audit/support/export/crawler/analytics 泄露构建和源码供应链凭据.
- [x] 扩展 launch billing/tax/accounting secret redaction/classification 覆盖 Paddle、Lemon Squeezy、Chargebee、Recurly、Braintree、PayPal、Adyen、TaxJar、Avalara、QuickBooks、Xero 的字符串 assignment 和结构化 metadata，避免 logs/traces/errors/audit/support/export/crawler/analytics 泄露付费、税务、会计凭据.
- [x] 扩展 launch observability webhook secret redaction/classification 覆盖 Healthchecks、Better Uptime、Grafana OnCall、Rollbar、Bugsnag、Axiom、Logz.io、Cronitor、UptimeRobot、Honeybadger、Airbrake、Scout APM、Lightstep、Chronosphere、SigNoz、Logflare、Sematext、Papertrail 的字符串 URL/token assignment 和结构化 metadata，避免 staging evidence、logs/traces/errors/audit/support/export/crawler/analytics 泄露监控和告警 webhook 凭据.
- [x] 扩展 launch export/render/design secret redaction/classification 覆盖 CloudConvert、ConvertAPI、Browserless、Browserbase、ScreenshotOne、Urlbox、HTML/CSS to Image、DocRaptor、PDFShift、PDF.co、Aspose、Cloudinary、Uploadcare、ImageKit、Filestack、Shotstack、Bannerbear、RenderForm、Figma 的字符串 assignment 和结构化 metadata，避免 PPT/Figma/thumbnail/export metadata、logs、traces、audit、support、analytics 泄露渲染与设计转换凭据.
- [x] 扩展 launch crawler/research secret redaction/classification 覆盖 Firecrawl、Tavily、Exa、SerpAPI/Serper、Apify、Bright Data、Zyte、ScrapingBee、ScraperAPI、Crawlbase、Diffbot、Jina、SearchAPI 的字符串 assignment 和结构化 metadata，避免 crawler findings、support、audit、traces、export、analytics 泄露外部检索和抓取凭据.
- [x] 扩展 launch deployment/edge/secret-manager redaction/classification 覆盖 Vercel、Netlify、Render、Railway、Fly.io、Cloudflare Pages/Tunnel/Turnstile、reCAPTCHA、hCaptcha、Doppler、Infisical、1Password service account、Vault、SOPS 的字符串 assignment 和结构化 metadata，避免 deployment evidence、logs、traces、audit、support、export、analytics 泄露部署平台、边缘保护和 secret manager 凭据.
- [x] 扩展 malware scanner provider secret redaction/classification 覆盖 VirusTotal、Google Safe Browsing、OPSWAT/MetaDefender、ClamAV/ICAP、YARA signing keys、CrowdStrike、SentinelOne、Sophos、Microsoft Defender 的字符串 assignment、结构化 metadata、HTTP header、slog message/attrs，避免 upload scan、audit、support、export、crawler、analytics metadata 泄露扫描供应商凭据.
...
- [x] 硬化 upload malware scan trust boundary：user upload metadata cannot spoof scanner provider/status/definition or trigger placeholder suspicious outcomes; scanner request metadata is allowlisted to non-secret workflow context and tests cover service/server paths。
...
- [x] Release gate evidence basename exact-path guard 通过：`scripts/validate_stage0_rev2.py` rejects release gate evidence refs that mention validator-owned artifact basenames without the full canonical path, so sibling artifacts or shorthand filenames cannot imply launch-gate closure。
- [x] Release gate check/condition existing-file guard 通过：`scripts/validate_stage0_rev2.py` validates every existing concrete file cited by release-gate check and Do-Not-Launch evidence refs for gate-specific environment, passable status, and non-closure probe context, so stale or cross-gate JSON cannot support subitem closure while allowed definition directories remain definition-only。
- [x] Release gate broad runtime evidence directory guard 通过：`scripts/validate_stage0_rev2.py` rejects release gate evidence refs that cite bare `.github/workflows/`、`ops/evidence/ci/`、`ops/evidence/staging/`、or `ops/evidence/production/` directory placeholders, so launch gates require exact installed workflow/runtime evidence files instead of broad directory prose。
- [x] Release gate fixture identity guard 通过：`scripts/validate_stage0_rev2.py` rejects extra or missing `fixtures/stage0/rev2/release_gate_evidence.*.json` files and requires each canonical file to match its validator-owned `gate` and `evidence_id`, so copied, renamed, or sibling fixtures cannot contribute to Local Alpha、CI、Private Beta/Staging、Production、or Do-Not-Launch closure。
- [x] Runtime gate_impact closed-schema guard 通过：`scripts/validate_stage0_rev2.py` rejects unknown `gate_impact` keys in runtime evidence, so new launch-clearance metadata cannot bypass validator-owned checklist rows, gate ownership, or preserved-blocker policy。
- [x] Runtime gate_impact release-gate check-id match guard 通过：`scripts/validate_stage0_rev2.py` rejects runtime evidence whose nested `gate_impact.release_gate_check_id` disagrees with the artifact top-level `release_gate_check_id`, so copied gate metadata cannot close the wrong Local Alpha、CI、Private Beta/Staging、Production, or Do-Not-Launch row。
- [x] Runtime gate_impact false-readiness term guard 通过：`scripts/validate_stage0_rev2.py` rejects runtime evidence `gate_impact` values containing launch-ready、ready for production、go for production、上线就绪、生产就绪、or other no-go false-readiness terms unless the value is an exact validator-owned checklist row, so blocked runtime artifacts cannot smuggle launch clearance through prose。
- [x] Private Beta/Staging cleared-condition state guard 通过：`scripts/validate_stage0_rev2.py` computes support/retry/abuse and crawler approval/provenance Do-Not-Launch condition states from exact staging runtime evidence, so cleared fixture rows cannot drift outside the validator-owned launch-condition map。
- [x] Release gate decision status-token guard 通过：`scripts/validate_stage0_rev2.py` requires every `gate_decision.evidence_ref` to include the exact `gate_decision.status=go|no_go` token for its own fixture, and production upstream dependency refs must name upstream `gate_decision.status`, so fixture existence cannot be misread as Local Alpha、CI、Private Beta/Staging、Production、or Do-Not-Launch clearance。
- [x] Release gate fixture dependency status-token guard 通过：`scripts/validate_stage0_rev2.py` rejects release-gate evidence refs that cite CI、Private Beta/Staging、Production gate fixture paths without the exact cited fixture `gate_decision.status=go|no_go` token, so upstream dependency fixture presence cannot preserve or clear launch gates by itself。
...
- For any existing path cited by a `gate_decision.evidence_ref`, the validator must resolve that path as an exact file and validate its gate-specific status before the decision prose may treat it as present/pass; existing directories, stale runtime JSON, blocked probe JSON, and non-passable installed workflow files are invalid as decision evidence.
...
- Passed runtime gate checks must not cite runtime evidence directories as supporting pass evidence; after the exact files exist, evidence refs may name only the validator-owned file paths for the runtime proof, because directory existence cannot prove environment、status、release_gate_check_id、gate_impact、or preserved-blocker semantics.
...
- A top-level gate checklist item, aggregate runtime checklist item, global Do-Not-Launch item, or fixture-level `go` decision also cannot cite any runtime JSON with a non-passing `status`, even when that JSON appears only in `gate_decision.evidence_ref`; blocked probe evidence must remain confined to explicit non-closure probe rows.
...
- Release gate fixture `schema_version` must remain `stage0.rev2`, `gate` must match the filename's canonical gate, `provenance.created_by_lane` must remain `lane6`, and `provenance.blueprint_sections` must be a non-empty list restricted to Sections 23/24 release-gate policy references; a fixture with the right checklist prose but wrong identity/provenance is invalid.
- Release gate fixture `checks[*].evidence_ref`, `do_not_launch_checks[*].evidence_ref`, and `gate_decision.evidence_ref` must not cite README、`Docs/stage0_blueprint.md`、`Docs/stage0_draft.md`、issues、聊天记录、conversation logs、or debating-runs as launch-closure evidence; release gates may rely only on this Rev2-owned fixture set, validator-owned scripts, exact runtime evidence, exact deployment artifacts, and the production upstream dependency exception below.
- Release gate fixture evidence refs must not cite `Docs/stage0_blueprint_rev2.md` itself as pass/clearance evidence; Rev2 is the authoritative requirements source, not runtime/deployment proof, and doc-only launch closure remains invalid.
- A release gate evidence ref may not cite another `fixtures/stage0/rev2/release_gate_evidence.*.json` file as direct runtime proof; the only exception is Production `ci_staging_gates_not_passed`, which may cite CI and Private Beta/Staging gate fixtures solely as upstream dependency status evidence and must name their `gate_decision.status`, not treat their presence as clearance.
- Every release gate fixture `gate_decision.evidence_ref` must include the exact self status token `gate_decision.status=go` or `gate_decision.status=no_go`, so fixture presence cannot be misread as aggregate gate or Do-Not-Launch clearance.
- Release-gate evidence refs that cite CI、Private Beta/Staging、Production gate fixture paths must include the exact cited fixture status token, such as `fixtures/stage0/rev2/release_gate_evidence.ci.json gate_decision.status=no_go`; fixture path presence without the status token cannot preserve or clear launch gates.
- Upstream dependency fixture status tokens are evidence of dependency state only; they cannot substitute for the exact runtime/deployment artifacts required by the current gate, and they cannot clear Production or global Do-Not-Launch readiness until the cited fixture status is `go`.
- Every release gate check, Do-Not-Launch condition, and gate decision has a validator-owned evidence path allowlist; an evidence_ref that cites a concrete repo path outside that check/condition/decision allowlist is invalid even if the path exists, is under `ops/evidence/`, or passes another gate's validator.
- The evidence path allowlist is closed-world per check/condition/decision: adding a new runtime/deployment artifact for launch closure requires updating the validator-owned mapping and the corresponding concrete checklist row in the same change, so broad `ops/evidence/` path substitution cannot silently close Local Alpha、CI、Private Beta/Staging、Production、or global Do-Not-Launch readiness.
- Release gate broad runtime evidence directory guard 通过：`scripts/validate_stage0_rev2.py` rejects release gate evidence refs that cite bare `.github/workflows/`、`ops/evidence/ci/`、`ops/evidence/staging/`、or `ops/evidence/production/` directory placeholders, so launch gates require exact installed workflow/runtime evidence files instead of broad directory prose。
- Release gate evidence refs that cite bare `.github/workflows/`、`ops/evidence/ci/`、`ops/evidence/staging/`、or `ops/evidence/production/` directory placeholders are invalid; launch closure and blocker preservation must name exact validator-owned files such as `.github/workflows/stage0-rev2-ci.yml` or `ops/evidence/staging/object-storage-retention-cleanup.json` when those files are the required evidence.
- Release-gate evidence refs that mention a validator-owned artifact basename without its full canonical path are invalid; `public-paid-real-generation-claims.json`, `billing-refund-credit-webhook.json`, `backup-restore.json`, or any sibling shorthand cannot imply launch-gate closure unless the exact validator-owned path is cited.
...
- Local Alpha `gate_decision.evidence_ref` may not use broad `ops/evidence/local_alpha/` directory shorthand after the gate computes `go`; it must cite the exact twelve workflow API、Playwright、and export ZIP runtime evidence files so the top-level Local Alpha gate cannot be closed by directory existence alone.
...
- CI runtime checklist rows and release-gate checks cannot remain open/blocked after exact installed-workflow/runtime evidence becomes passable: `.github/workflows/stage0-rev2-ci.yml` must contain Stage 0 CI tokens, each exact `ops/evidence/ci/*.json` runtime file must declare `environment=ci`, matching `release_gate_check_id`, passing status, no preserved blockers, and required PR/main、Playwright、Docker semantics.
...
- README launch-readiness prose must mirror release gate fixture decisions: it may report Local Alpha as go only while `fixtures/stage0/rev2/release_gate_evidence.local_alpha.json` computes `go`, must report CI、Private Beta/Staging、Production Launch、and global Do-Not-Launch as no-go while their fixtures remain blocked, and must name the exact blocked check IDs, active Do-Not-Launch condition IDs, missing/present runtime artifact paths, and upstream fixture `gate_decision.status` dependencies for each open gate; README prose cannot be used as launch evidence.
- README no-go rows must not summarize away split gate blockers: the Private Beta/Staging row must name `staging_object_storage_signed_downloads`, `object_storage_signed_retention_runtime_missing`, the present signed URL evidence file, and the missing retention cleanup evidence file; the Production row must name `production_backup_rollback_incident`, `backup_restore_rollback_smoke_missing`, `production_deploy_rollback_smoke_missing`, `ci_staging_gates_not_passed`, the missing production backup/restore and rollback/post-deploy evidence files, and the exact upstream CI/Staging fixture `gate_decision.status` values.
...
- Runtime evidence `gate_impact` metadata cannot claim closure for a top-level Local Alpha、CI、Private Beta/Staging、Production、or global Do-Not-Launch checklist item unless that exact checklist row is checked and the authoritative release gate fixture already computes ready; a single `ops/evidence/**/*.json` file cannot override an open aggregate row, active Do-Not-Launch condition, or fixture-level `no_go` decision.
- A runtime evidence file's `gate_impact` cannot claim another gate's top-level checklist row; Local Alpha evidence cannot clear CI/Staging/Production rows, staging evidence cannot clear Production rows, production evidence cannot clear CI/Staging rows, and global Do-Not-Launch closure still requires every release gate fixture to compute `go`.
...
- `gate_impact` partial evidence may mention checked check-level rows or definition-only rows, but it cannot use `can_clear_*` flags to close an aggregate gate, sibling gate, or global Do-Not-Launch row while the release gate fixture still has blocked checks or active conditions.
...
- Active split Do-Not-Launch condition evidence that cites an existing exact JSON artifact must validate that artifact's environment, release gate check ID, passing status, and row-specific semantics before the fixture may describe it as present/passed; an existing but wrong-environment or wrong-check artifact cannot preserve or clear a blocker.
- Cleared split Do-Not-Launch condition evidence must cite every exact validator-owned split artifact, every artifact must exist, and the evidence text must not describe any cited existing artifact as absent/missing; cleared provider/claims, billing, production backup/rollback, object-storage, CI runtime, or upstream CI/Staging dependency conditions cannot rely on sibling artifacts or prose.
- Cleared split Do-Not-Launch condition evidence for upstream dependency fixtures must require those fixture `gate_decision.status` values to be `go`; production `ci_staging_gates_not_passed` cannot clear merely because the CI and Private Beta/Staging fixture files exist.
...
- A combined split checklist row, such as Private Beta/Staging object storage、Private Beta/Staging legal/support、Production provider、Production billing、Production backup/rollback、or Production legal/support, may close only when every exact split evidence file is passable, every validator-owned split evidence row is checked, the combined release-gate check is `pass`, and every mapped Do-Not-Launch condition for that check is false.
- A combined split checklist row must remain open while any exact split evidence row is open, any exact split artifact is missing/non-passable, the combined release-gate check is blocked/failing, or any mapped Do-Not-Launch condition is active; checked combined rows cannot hide split launch blockers.
- If every exact split evidence file is passable, every validator-owned split row is checked, the combined release-gate check passes, and no mapped Do-Not-Launch condition is active, the combined split checklist row must be checked in the same change; stale open combined rows are invalid except where an explicit upstream dependency condition remains active.
...
- CI no-go gate-decision evidence that cites `.github/workflows/stage0-rev2-ci.yml` as present/pass must fail unless that exact installed workflow file exists and contains Stage 0 Rev2 validation, Playwright smoke, Docker build, and `validate_stage0_rev2.py` coverage; the ops draft workflow under `ops/ci/` cannot satisfy the installed-workflow slot.
