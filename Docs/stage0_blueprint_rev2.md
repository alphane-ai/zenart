...
- A top-level gate checklist item may close only after its aggregate runtime checklist item is closed, every concrete aggregate subitem is checked, the matching release fixture has no blockers, and `gate_decision.status` is `go`; if those are all true, the gate checklist item must be updated in the same change.
...
- Private Beta/Staging remains open until external-user staging runtime evidence exists for object storage signed downloads, observability/backup/load, legal page visibility, and the remaining aggregate workflow evidence.
