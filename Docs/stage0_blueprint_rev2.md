...
- Release gate fixture `schema_version` must remain `stage0.rev2`, `gate` must match the filename's canonical gate, and `provenance.created_by_lane` must remain `lane6`; a fixture with the right checklist prose but wrong identity/provenance is invalid.
...
- Blocked Local Alpha workflow smoke evidence must also name the exact missing per-workflow files for every still-open workflow row: `ops/evidence/local_alpha/local_merchant_campaign_pack.api_smoke.json`、`ops/evidence/local_alpha/local_merchant_campaign_pack.playwright_happy_path.json`、`ops/evidence/local_alpha/local_merchant_campaign_pack.export_zip.json`、`ops/evidence/local_alpha/character_ip_concept_pack.api_smoke.json`、`ops/evidence/local_alpha/character_ip_concept_pack.playwright_happy_path.json`、and `ops/evidence/local_alpha/character_ip_concept_pack.export_zip.json`; broad `ops/evidence/local_alpha/` blocker prose cannot preserve the aggregate Local Alpha blocker.
