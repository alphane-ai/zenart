...
- [x] README launch-readiness snapshot guard 通过：README states Local Alpha is go, keeps CI、Private Beta/Staging、Production Launch、and Do-Not-Launch no-go with exact missing runtime artifact paths, and validator rejects README prose that implies those gates are launch-ready before release fixtures compute `go`。
